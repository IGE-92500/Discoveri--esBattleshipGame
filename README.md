# DiscoveriesBattleshipGame

## Grupo: TP06-8

| Curso   | Nº Aluno   | Nome               |
|--------|------------|--------------------|
| IGE    | 92500      | Nuno Venâncio      |
| IGE    | 110886     | Marta Fonseca      |
| IGE    | 111105     | Guilherme Calção   |
| IGE    | 105446     | Afonso Ramos       |

 ## Tipos de Navios 

| Nome Atual        | Descobrimentos | Nome (EN) | Tamanho | Quantidade |
|------------------|----------------|-----------|---------|------------|
| Porta-aviões     | Galeão         | Galleon   | 5       | 1          |
| Navio 4 canhões  | Fragata        | Frigate   | 4       | 1          |
| Navio 3 canhões  | Nau            | Carrack   | 3       | 2          |
| Navio 2 canhões  | Caravela       | Caravel   | 2       | 3          |
| Submarino        | Barca          | Barge     | 1       | 4          |


## Regras do Jogo

- Cada jogador tem duas grelhas 10x10:
  - a sua frota - o tabuleiro do adversário
- Os navios:
  - são colocados na horizontal ou vertical  
  - não se podem tocar entre si  
- Em cada ronda, o jogador dispara três tiros
- O adversário indica:
  - tiros na água  
  - navios atingidos  
- O jogador regista os resultados no seu tabuleiro de ataque  
- Ganha quem afundar todos os navios do adversário
CI test
CI test
